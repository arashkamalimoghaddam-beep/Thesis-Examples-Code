...
for base in CandidateBasis:
    # بردارهای مربوط به دلتا 1 (c1, c2, c3, c4)
...
# تخصیص حافظه اولیه برای ماتریس (حالا 5 ستون دارد)
num_points = len(eta_points)
A_mat = np.empty((num_points, 5))

# تغییر به انتخاب 4 پایه برای دلتا 1
for idx_combo_d1 in combinations(range(len(CandidateBasis)), 4):
    for idx_d2 in range(len(CandidateBasis)):
        ...
        # 2. جایگذاری مستقیم ستون‌ها (4 ستون برای دلتا 1، 1 ستون برای دلتا 2)
        ...
        A_mat[:, 4] = candidate_vectors_d2[idx_d2]
...
delta2_sym = best_coeffs[4] * selected_base_delta2
...
F_final_sym += best_coeffs[4] * candidate_funcs_d2[best_d2_index]

print("\n" + "="*60)
print("SELECTION INFORMATION (Fully Automated & Optimized):")
print(f"Bases chosen for Delta 1 (c1, c2, c3, c4): {selected_bases_delta1}")
print(f"Basis chosen for Delta 2 (c5): {selected_base_delta2}")
