...
class ChelyshkovCollocationSolver:
    ...
    def solve(self):
        ...
        # در این مثال شرط اولیه F(0) = 1 است، پس c(x) = 1.0
        c_expr = sp.sympify(1.0)
        C_vec = self.project_function(c_expr, Q_inv)
        C_mat = sp.Matrix(C_vec).T

        # 3. محاسبه بردار G (تصویر ترم غیر همگن)
        # در معادله ما g(x) = 0.5 * (e^{-2} - 1)
        g_expr = sp.sympify(0.5 * (sp.exp(-2) - 1))
        ...
        g_sym = (G_mat * Phi_sym)[0]  # استفاده از \tilde{g}

        # 4. نقاط هم‌محلی بر اساس ریشه‌های چندجمله‌ای پایه Ce_{N+1, 0}
        ...
        for l in range(self.M):
            ...
            integral_sym += w_l[l] * (F_val ** 2)

        # رابطه باقیمانده: F'(\eta) + F(\eta) - g(\eta) - \int_0^1 F(\tau)^2 d\tau = 0
        R_sym = F_prime_sym + F_sym - g_sym - integral_sym
...
if __name__ == "__main__":
    # با افزایش M دقت ترم غیرخطی تضمین می‌شود
    N_degree = 4
    solver = ChelyshkovCollocationSolver(N=N_degree, M=30)
    solver.solve()

    table_points = np.array([0.0, 0.16, 0.32, 0.48, 0.64, 0.80, 0.96, 1.0])

    approx_vals = solver.evaluate(table_points)
    exact_vals = np.exp(-table_points)
    ...
    print(r"    \centering")
    print(r"    \caption{Comparison of Exact and Approximate Solutions for $N=" + str(N_degree) + r"$}")
    ...
    plt.subplot(1, 2, 1)
    plt.plot(x_test, y_exact, 'b-', linewidth=2, label=r'Exact: $F(\eta) = e^{-\eta}$')
