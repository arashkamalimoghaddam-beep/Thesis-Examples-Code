...
# ==========================================
n = 11
# بازه انتگرال در معادله جدید از 0 تا 1 است
...
# ---> تعریف کرنل بازتولیدکننده W_2^2 سازگار با شرط اولیه v(0)=0
# کرنل استاندارد فضای W_2^2 برای معادلات مرتبه اول
K_1 = x_sym*y_sym + 0.5 * x_sym * y_sym**2 - (y_sym**3)/6
K_2 = x_sym*y_sym + 0.5 * (x_sym**2) * y_sym - (x_sym**3)/6
...
def apply_L_star(expr, var):
    ...
    عملگر الحاقی آن معادل 1 بار مشتق‌گیری است.
    """
...
def int_t_w(m):
    ...
    # بازه انتگرال‌گیری از 0 تا 1
    int1 = sp.integrate(t_sym * w_2_t, (t_sym, 0, xm)).doit()
    int2 = sp.integrate(t_sym * w_1_t, (t_sym, xm, 1)).doit()

    return float(int1 + int2)

# تبدیل توابع نمادین به توابع عددی (NumPy) برای سرعت بالا در حلقه
w_num_func = sp.lambdify((x_sym, y_sym), w_sym, modules=['numpy'])
w_deriv_sym = sp.diff(w_sym, x_sym, 1) # مشتق اول برای بررسی باقیمانده F'
...
# با اعمال تغییر متغیر v(x) = F(x) - 1 فرمول سمت راست (f) به این شکل درمی‌آید:
# v'(x) - x^2 \int_0^1 \tau v(\tau) d\tau = x^2(2e^{-1} - 0.5) - e^{-x}
def f_func(x):
    return (x**2) * (2 * np.exp(-1) - 0.5) - np.exp(-x)

for i in range(n):
    # فرمول f(x) بر اساس تغییر متغیر
    B_f_i = sum(beta[i, k] * f_func(x_nodes[k]) for k in range(i + 1))
    
    # ضریب پشت عبارت انتگرالی درون معادله (x^2)
    B_x_i = sum(beta[i, k] * (x_nodes[k]**2) for k in range(i + 1))
...
# اعمال بازگشت تغییر متغیر v(x) به F(x)
approx_plot = un_approx(x_plot) + 1.0
error_plot = np.abs(exact_plot - approx_plot)

# محاسبه مقدار انتگرال \int_0^1 \tau F_n(\tau) d\tau
int_tu_n = sum(A_coeffs[m] * int_t_w(m) for m in range(n))
integral_F = int_tu_n + 0.5 # چون F(x) = v(x) + 1 و انتگرال t*1 در بازه 0 تا 1 برابر 0.5 است

# باقی‌مانده معادله دیفرانسیل اصلی تصویر: 
# F'(x) = x^2(2e^{-1} - 1) - e^{-x} + x^2 * \int \tau F(\tau) d\tau
RHS_eq = (x_plot**2) * (2 * np.exp(-1) - 1) - np.exp(-x_plot) + (x_plot**2) * integral_F
LHS_eq = un_deriv_approx(x_plot) # مشتق ثابت 1 برابر صفر است، پس F' = v'
...
df_results['Exact Solution'] = df_results['Exact Solution'].apply(lambda val: f"{val:.15f}")
df_results['RKHS Solution'] = df_results['RKHS Solution'].apply(lambda val: f"{val:.15f}")
df_results['Absolute Error'] = df_results['Absolute Error'].apply(lambda val: f"{val:.2e}")

print("\n--- Numerical Results (Linear Fredholm IDE Extraction) ---")
