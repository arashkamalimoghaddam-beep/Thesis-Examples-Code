...
for base in CandidateBasis:
    # بردارهای مربوط به دلتا 1 (c1, c2, c3, c4, c5, c6)
...
# تخصیص حافظه اولیه برای ماتریس (حالا 7 ستون دارد)
num_points = len(eta_points)
A_mat = np.empty((num_points, 7))

# تغییر به انتخاب 6 پایه برای دلتا 1
for idx_combo_d1 in combinations(range(len(CandidateBasis)), 6):
    for idx_d2 in range(len(CandidateBasis)):
        ...
        # 2. جایگذاری مستقیم ستون‌ها (6 ستون برای دلتا 1، 1 ستون برای دلتا 2)
        ...
        A_mat[:, 5] = candidate_vectors_d1[idx_combo_d1[5]]
        A_mat[:, 6] = candidate_vectors_d2[idx_d2]
...
delta1_sym = (best_coeffs[0] * selected_bases_delta1[0] + 
              ...
              best_coeffs[5] * selected_bases_delta1[5])
delta2_sym = best_coeffs[6] * selected_base_delta2
...
F_final_sym += best_coeffs[5] * candidate_funcs_d1[best_d1_indices[5]]
F_final_sym += best_coeffs[6] * candidate_funcs_d2[best_d2_index]

print("\n" + "="*60)
print("SELECTION INFORMATION (Fully Automated & Optimized):")
print(f"Bases chosen for Delta 1 (c1, c2, c3, c4, c5, c6): {selected_bases_delta1}")
...
print(f"Coefficient c7: {best_coeffs[6]:.6f}")
print("-" * 60)
...
best_A_mat = np.column_stack([
    ...
    candidate_vectors_d1[best_d1_indices[5]],
